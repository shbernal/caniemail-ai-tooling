...
Re-running at an already-published version skips rather than fails, so a release
that half-failed can simply be run again.
...
`mcp/package.json`, so there is no third place to remember. The workflow refuses
to publish a tag that disagrees with either manifest.
...
Then confirm the published artifacts run, rather than assuming a green workflow
implies it:
...
`npm view` lags a publish by a minute or two, so a version that just went up
still reads as the previous one. Check the release page or the registry JSON
directly rather than concluding the upload failed.

## Authentication

- **npm** uses [trusted publishing][tp], which is OIDC. The package is
  configured on npmjs.com to trust this repository's `publish.yml`, so the
  workflow needs `id-token: write` and no token at all. npm attaches build
  provenance as a side effect. The job upgrades npm before publishing because
  OIDC needs `>= 11.5.1`, which is newer than any Node line currently bundles.
- **ClawHub** has no OIDC path, so it authenticates with a long-lived
  ...
  outside the environment reads that secret as an empty string. A job inside it
  carries an `environment` claim that npm's trusted publisher, configured
...
Drop `--dry-run` to publish. Pass the source references so the listing links
back to the commit it was built from, and pass `--changelog`. Without it ClawHub
...
to `src` and `README.md`, and `src/data/caniemail.json` rides along inside
`src`. Verify that, because a tarball without the snapshot has no offline
fallback.
...
`workspace:` dependencies for a publish step to rewrite, because it reaches the
core by file copy. Keeping the publish command the same as the one consumers
see, `npx -y mcp-server-caniemail`, is worth more than tool consistency.
